...
#include "Assets/GrassPhysics/Shaders/Core/GrassSettings.cginc"
#include "Assets/GrassPhysics/Shaders/Core/GrassPhysics.cginc"
...
float2 GrassDoWind(inout float4 color, float4 vertex)
{
	...
	float4 tintColor = TerrainWaveGrass(vertexCopy, waveAmount, color);
	color = float4(lerp(color.rgb, tintColor.rgb, _WavingTint.a), color.a);
...
#include "Assets/GrassPhysics/Shaders/Core/GrassSurface.cginc"

// Used for StandardSimpleLighting shader
half4 LitPassFragmentGrass(GrassVertexOutput input) : SV_Target
{
	return surf(input);
    /* //Default grass fragment shader used by Unity
